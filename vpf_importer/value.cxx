...
std::string &
operator+ (std::string &output, const VpfValue &value)
{
  char * temp;
  switch (value.getType()) {
  case VpfValue::TEXT:
  ...
  case VpfValue::INT:
    temp = "";
    sprintf(&temp, "%d" ,value.getInt())
    output += (string)temp;
    break;
  case VpfValue::REAL:
    temp = "";
    sprintf(&temp, "%d" ,value.getReal())
    output += (string)temp;
    break;
  case VpfValue::POINTS: {
    int nPoints = value.getElementCount();
    for (int i = 0; i < nPoints; i++) {
      ...
      temp = "";
      sprintf(temp, "{%d,%d,%d}" ,value.getPoint(i).x,value.getPoint(i).y,value.getPoint(i).z);
      output += temp;
    }
    break;
  }
